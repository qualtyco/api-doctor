...
Upload validation gaps, hardcoded API keys in injected scripts, and missing SRI hashes.
...
| Rule | Broken | Fixed |
| ---- | ------ | ----- |
| upload-validate-fn-void | `upload-plugin-broken.tsx`, `upload-handler-broken.ts` | `upload-plugin-fixed.tsx`, `validate-handler-adversarial.tsx` |
| script-src-hardcoded-api-key | `desmos-node-broken.ts`, `embed-node-broken.ts` | `desmos-node-fixed.ts`, `embed-node-adversarial.ts` |
| dynamic-script-no-sri | `desmos-node-broken.ts`, `analytics-script-broken.ts` | `desmos-node-fixed.ts`, `script-removed-adversarial.ts` |

---

### Correctness

HTML round-trip breakage from missing `renderHTML`, undo-stack pollution, and atom-node command misuse.
...
| Rule | Broken | Fixed |
| ---- | ------ | ----- |
| addAttributes-missing-renderHTML | `formula-node-broken.tsx`, `example-node-broken.tsx` | `formula-node-fixed.tsx`, `shorthand-attribute-adversarial.tsx` |
| appendTransaction-add-to-history | `definition-node-broken.tsx`, `spacing-plugin-broken.ts` | `definition-node-fixed.tsx`, `readonly-appendTransaction-adversarial.ts` |
| atom-node-wrap-in | `desmos-node-broken.ts`, `embed-node-broken.ts` | `desmos-node-fixed.ts`, `container-node-adversarial.ts` |
| drop-handler-pos-precedence | `upload-images-broken.tsx`, `drop-handler-broken.ts` | `upload-images-fixed.tsx`, `correct-precedence-adversarial.tsx` |

---

### Reliability

Performance issues from O(n) document scans on every transaction.
...
| Rule | Broken | Fixed |
| ---- | ------ | ----- |
| appendTransaction-full-scan | `definition-node-broken.tsx`, `validator-plugin-broken.ts` | `definition-node-fixed.tsx`, `no-scan-adversarial.ts` |

---

### Integration

Regex coverage gaps, table extension wiring, and missing markdown serialization.
...
| Rule | Broken | Fixed |
| ---- | ------ | ----- |
| twitter-url-regex | `twitter-extension-broken.tsx`, `slash-command-broken.tsx` | `twitter-extension-fixed.tsx`, `github-regex-adversarial.tsx` |
| prefer-table-kit | `extensions-broken.ts`, `editor-setup-broken.tsx` | `extensions-fixed.ts`, `table-only-adversarial.ts` |
| tiptap-markdown-missing-node-spec | `mathematics-broken.ts`, `custom-node-broken.ts` | `mathematics-fixed.ts`, `markdown-already-set-adversarial.ts` |
