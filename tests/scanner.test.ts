...
describe('scan', () => {
  it('flags the 4 broken Resend webhook signature fixtures', async () => {
    ...
    const expected = [
      ...
      'resend-broken-wrapper-import-no-verify.ts',
    ].sort();
  ...
  it('does not flag any of the correct fixtures for the webhook-signature rule', async () => {
    const { results } = await scan(fixturesDir);
    ...
    const correct = [
      ...
      'resend-ok-native-verify.ts',
      'resend-ok-native-verify-direct-import.ts',
      'resend-ok-doc-page-embedded-code-string.tsx',
    ];
