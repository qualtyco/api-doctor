...
async function detectProviders(directory, filesContent) {
  ...
  let deps = {};
  try {
    const raw = await (0, import_promises.readFile)((0, import_node_path.join)(directory, "package.json"), "utf-8");
...
async function walk(dir, root, files) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      files.push((0, import_node_path2.relative)(root, full));
    }
  }
}
...
async function scan(directory, options = {}) {
  const absRoot = (0, import_node_path2.resolve)(directory);
  const paths = [];
  await walk(absRoot, absRoot, paths);
  const filesContent = /* @__PURE__ */ new Map();
  for (const rel of paths) {
    const content = await (0, import_promises2.readFile)((0, import_node_path2.join)(absRoot, rel), "utf-8");
  ...
  const tmpDir = (0, import_node_fs.mkdtempSync)((0, import_node_path2.join)(import_node_os.default.tmpdir(), "api-doctor-oxlint-"));
  const configPath = (0, import_node_path2.join)(tmpDir, "oxlintrc.json");
  const config = {
  ...
  try {
    ...
    for (const d of diagnostics) {
      ...
      const relFile = (() => {
        ...
        if (!filename) return "";
        if (filename.startsWith(absRoot)) return (0, import_node_path2.relative)(absRoot, filename);
...
// src/cli.ts
var __dirname = (0, import_node_path3.dirname)((0, import_node_url.fileURLToPath)(importMetaUrl));
var pkg = JSON.parse(
  (0, import_node_fs2.readFileSync)((0, import_node_path3.join)(__dirname, "../package.json"), "utf-8")
);
